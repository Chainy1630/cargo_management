...
//   static findBestFit(itemDims, freeSpaces) {
//     for (const space of freeSpaces) {
//       for (const rotation of itemDims) {
//         if (rotation.width <= space.width &&
...
//     for (const space of freeSpaces) {
//       // Check if the space is affected by the placement
//       if (space.x < placement.position.start.width &&
//           space.y < placement.position.start.depth &&
//           space.z < placement.position.start.height) {
//         // Create a new free space for the remaining area
//         updatedSpaces.push({
//           x: space.x,
//           y: space.y,
//           z: space.z,
//           width: placement.position.start.width - space.x,
//           height: placement.position.start.depth - space.y,
//           depth: placement.position.start.height - space.z
//         });
//       }

//       // Check if the space is larger than the placement
//       if (space.x + space.width > placement.position.end.width &&
//           space.y + space.height > placement.position.end.depth &&
//           space.z + space.depth > placement.position.end.height) {
//         updatedSpaces.push({
//           x: placement.position.end.width,
//           y: placement.position.end.depth,
//           z: placement.position.end.height,
//           width: space.x + space.width - placement.position.end.width,
//           height: space.y + space.height - placement.position.end.depth,
//           depth: space.z + space.depth - placement.position.end.height
//         });
//       }
//     }
...
//   const existingLogs = await Log.find({actionType:"retrieval"});

//   // If the collection is not empty, clear it
//   if (existingLogs.length > 0) {
//     await Log.deleteMany({actionType:"retrieval"}); // Remove all existing logs
...
//     await Placement.deleteMany({}); // Remove all existing placements
...
//   try {
//      const existingPlacement=await Placement.find();
//      const item=await Item.find();
     
//      if(existingPlacement.length===item.length)
//      {
//         res.status(200).json({placements:existingPlacement});
//         return;
//      }
//     const placements = await getplaced();
...
// Main function to place items
const getPlacedItems = async () => {
  const items = await Item.find().sort({ priority: -1 }); // Sort items by priority
  ...
  for (const container of containers) {
    ...
    for (const item of items) {
      const itemDims = OptimizedBinPacker.generateRotations(item);
      const placement = OptimizedBinPacker.findBestFit(
        itemDims,
        freeSpaces,
        item.priority
      );
      ...
      if (placement) {
        ...
        const savedPlacement = new Placement(newPlacement);
        await savedPlacement.save();

        placements.push(savedPlacement);
        freeSpaces = OptimizedBinPacker.updateFreeSpaces(freeSpaces, placement);
      }
    }
  }
