...
exports.rearrangeItem = async (req, res) => {
  ...
  // Check if the result is null or an empty array
    ...
    // For each container, find the items placed in that container
    ...
    //   if (remaingfrom.length === 0) {
    //     console.log("No items remaining after processing.");
    //     break;
    //   }
      

    //   for (const item of remaingfrom) {
    ...
    //       const { placements: pts, remaining: rfrom } = await getplaced(itemIdsInContainer, container.containerId);

    //       // Check if the item exists in the Placement database
    //       const placementEntry = await Placement.findOne({ itemId: item.itemId });

    //       // If the containerId is not "Not Placed", remove it from remaingfrom
    ...
    //         const index = remaingfrom.indexOf(item);
    //         if (index > -1) {
    //           remaingfrom.splice(index, 1); // Remove the item from remainto
    //         }
    //       }
    //     }
